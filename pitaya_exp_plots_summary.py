...
def generate_gene_exp_figure( figfile, genes, exp, gene_order, sample_groups, genotype_order, border ):
	...
	ax.set_xlim( -0.5, max( [ x for sublist in positions.values() for x in sublist ] )+0.5 )
	ax2.set_xlim( -0.5, max( [ x for sublist in positions.values() for x in sublist ] )+0.5 )
	...
	ax.legend( handles=my_legend, loc="upper left", ncol=1, bbox_to_anchor=(0.45, 0.95), fontsize=10 )
	
	plt.subplots_adjust( left=0.075, right=0.999, top=0.99, bottom=0.13, hspace=0.03  )
