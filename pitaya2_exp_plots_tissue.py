...
__usage__ = """
					python pitaya2_exp_plots_tissue.py
					--genes <GENES_FILE>
					--exp <EXPRESSION_FILE>
...
def generate_gene_exp_figure( figfile, genes, exp, gene_order, sample_groups, genotype_order, upper_border, lower_border ):
	...
	cultivar_colors = { "GW-peel":"green", "YW-peel":"gold", "RR-peel":"deeppink", "GW-pulp":"lightgreen", "YW-pulp":"yellow", "RR-pulp":"red" }
	...
	ax2.text( -2.5, upper_border, "transcript abundance [TPMs]", fontsize=10, rotation=90, ha="center", va="center" )
	...
	my_legend = [	mpatches.Patch(color=cultivar_colors[ "RR-peel" ], label='RR-peel'),	#$\it{X}$ $\it{X}$ 
								mpatches.Patch(color=cultivar_colors[ "GW-peel" ], label='GW-peel'),	#$\it{X}$ $\it{X}$ 
								mpatches.Patch(color=cultivar_colors[ "YW-peel" ], label='YW-peel'),	#$\it{X}$ $\it{X}$ x $\it{X}$ 
								mpatches.Patch(color=cultivar_colors[ "RR-pulp" ], label='RR-pulp'),	#$\it{X}$ $\it{X}$ 
								mpatches.Patch(color=cultivar_colors[ "GW-pulp" ], label='GW-pulp'),	#$\it{X}$ $\it{X}$ 
								mpatches.Patch(color=cultivar_colors[ "YW-pulp" ], label='YW-pulp')	#$\it{X}$ $\it{X}$ x $\it{X}$ 
								#mpatches.Patch(color=cultivar_colors[ "BSJ" ], label='BSJ')
							]
	ax.legend( handles=my_legend, loc="upper right", ncol=2, bbox_to_anchor=(0.999, 0.999), fontsize=10 )
	
	plt.subplots_adjust( left=0.1, right=0.999, top=0.99, bottom=0.2, hspace=0.03  )
...
def main( arguments ):
	...
	sample_groups = { 'RR-peel': [ "SRR11603191", "SRR11603190", "SRR11603189" ],
									'RR-pulp': [ "SRR11603188", "SRR11603187", "SRR11603186" ],
									'YW-peel': [ "SRR11603195", "SRR11603194", "SRR11603185" ],
									'YW-pulp': [ "SRR11603181", "SRR11603180", "SRR11603179" ],
									'GW-peel': [ "SRR11603184", "SRR11603183", "SRR11603182" ],
									'GW-pulp': [ "SRR11603178", "SRR11603193", "SRR11603192" ]
								}
	#RR = re/red; YW=yellow/white, BR=green/white
	genotype_order = [ "RR-peel", "GW-peel", "YW-peel", "RR-pulp", "GW-pulp", "YW-pulp" ]	

	genes, gene_order = load_genes( gene_file )

	exp = load_exp( exp_file )

	generate_gene_exp_figure( figfile, genes, exp, gene_order, sample_groups, genotype_order, 10, 11 )
